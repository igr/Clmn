...
class DragTaskModel: ObservableObject {
    @Published var task: (TaskList, TaskGroup, Task)?
    ...
    func startDragOf(_ task: (TaskList, TaskGroup, Task), removeOnDrop: @escaping (_: Task) -> Void = { task in }) -> NSItemProvider {
        self.task = task
        self.removeOnDrop = removeOnDrop
        return NSItemProvider(item: task.1.name as NSString, typeIdentifier: TASK_UTI.identifier)
    }
